...
async function claimCount(sql: ReturnType<typeof getSql>, pieceId: string) {
  const rows = await sql`
    ...
    WHERE helmet_id = ${pieceId} OR hair_id = ${pieceId} OR head_id = ${pieceId}
       OR shirt_id = ${pieceId} OR pants_id = ${pieceId} OR accessory_id = ${pieceId}
  `;
...
export async function GET() {
  ...
  const sql = getSql();
  const rows = await sql`
    SELECT a.helmet_id, a.hair_id, a.head_id, a.shirt_id, a.pants_id, a.accessory_id, a.exclusive_id,
...
export async function POST(req: Request) {
  ...
  const accessory_id = body.accessory_id ?? null;
  const clearExclusive = body.clearExclusive !== false;

  const sql = getSql();

  const current = await sql`
    SELECT helmet_id, hair_id, head_id, shirt_id, pants_id, accessory_id, exclusive_id
  ...
  await sql`
    ...
    VALUES (
      ${session.id}, ${helmet_id}, ${hair_id}, ${head_id}, ${shirt_id}, ${pants_id}, ${accessory_id},
